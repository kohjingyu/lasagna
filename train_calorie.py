...
with open('calorie/recipe_ingredient_quantity_mapping.pickle','rb') as f:
    recipe_quantity_mapping = pickle.load(f)
...
if args.model_name == "resnet50":
    ...
    target_model = torchvision.models.densenet161(num_classes=num_classes, pretrained=True)
...
for epochs in range(total_epochs):
    ...
    for i, (input, target) in enumerate(train_loader):
        start = time.time()
        img_tensor, labels, labels2, recipe_id = get_tensor_from_data(input, target, class_mapping, recipe_quantity_mapping, dev_mode=dev_mode)
        ...
        target2 = labels2.to(device)
        output, output2 = target_model(img_tensor.float())
        print('output size: {}, output2: {}'.format(output.size(), output2.size()))
        print("Mean regression output", torch.mean(output2[output > 0]))
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            output, output2 = target_model(img_tensor.float())
            output = torch.sigmoid(output) #sigmoid values. since it's binary cross entropy.
